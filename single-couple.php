...
<article class="kp-single">
	...
	<header class="kp-single__head">
		<div class="kp-single__chips">
			<span class="kp-chip kp-chip--ink">♡ CP</span>
			<?php if ( $arc ) : ?>
				<a class="kp-chip" href="<?php echo esc_url( get_term_link( $arc ) ); ?>">#<?php echo esc_html( $arc->name ); ?></a>
			<?php endif; ?>
			<?php if ( $status ) : ?>
				<span class="kp-chip kp-chip--status kp-chip--status-<?php echo esc_attr( $status->slug ); ?>"><?php echo esc_html( $status->name ); ?></span>
			<?php endif; ?>
		</div>
	...
	<?php if ( has_post_thumbnail() ) : ?>
		<figure class="kp-single__hero">
			...
			<span class="kp-single__sticker">★ ♡ CP ♡ ★</span>
		</figure>
	<?php endif; ?>
	...
	$kp_sns = array(
		array( '📷 ' . get_post_meta( $pid, 'member_a', true ) . ' IG', get_post_meta( $pid, 'instagram_a', true ) ),
		array( '📷 ' . get_post_meta( $pid, 'member_b', true ) . ' IG', get_post_meta( $pid, 'instagram_b', true ) ),
	);
	$kp_sns_html = '';
	foreach ( $kp_sns as $s ) {
		if ( empty( $s[1] ) ) { continue; }
		$kp_sns_html .= '<a class="kp-profile__sns" href="' . esc_url( $s[1] ) . '" target="_blank" rel="noopener noreferrer">' . esc_html( trim( $s[0] ) ) . '</a>';
	}

	if ( $kp_rows || $kp_sns_html ) : ?>
	<div class="kp-profile">
		<div class="kp-profile__head">★ ♡ CP PROFILE ♡ ★</div>
		<?php if ( $kp_rows ) : ?>
	...
	<section class="kp-related">
		<h3 class="kp-section__head">★ 同じシーズンの CP ♡ ★</h3>
